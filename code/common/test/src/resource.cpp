...
//  The extract_resource function tests
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - //

namespace test::suite_extract_resource
{
...
class Resource_ExtractResource : public ::testing::TestWithParam<Param> {};

TEST_P(Resource_ExtractResource, Success)
{
    const Param& params = GetParam();
...
const std::vector<Param> g_success_scenarios_params{
    {
        {
